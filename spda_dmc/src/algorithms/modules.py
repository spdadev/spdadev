...
class SharedCNN(nn.Module):
	def __init__(self, obs_shape, num_layers=11, num_filters=32):
		...
		self.out_shape = _get_out_shape(obs_shape, self.layers)    # out_shape 21,21,3
		self.apply(weight_init)
...
class HeadCNN(nn.Module):
	def __init__(self, in_shape, num_layers=0, num_filters=32):    # in_shape: 21,21,3 为shared cnn的输出
		super().__init__()
		self.layers = []
