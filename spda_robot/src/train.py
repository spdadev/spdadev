...
def main(args):
    ...
    # Set seed
    utils_2.set_seed_everywhere(args.seed)
    ...
    work_dir = os.path.join(args.log_dir, args.domain_name+'_'+args.task_name+'_'+args.algorithm+'_'
                            +'1'+str(args.seed)+'_'+args.if_base+str(time.time()))
    print("Working directory:", work_dir)

    assert not os.path.exists(
        os.path.join(work_dir, "train.log")
    ), "specified working directory already exists"
    utils_2.make_dir(work_dir)
    tb_dir = work_dir + '//tb'
    writter = SummaryWriter(tb_dir)
    model_dir = utils_2.make_dir(os.path.join(work_dir, "model"))
    video_dir = utils_2.make_dir(os.path.join(work_dir, "video"))
    video = VideoRecorder(video_dir if args.save_video else None, height=448, width=448)
    utils_2.write_info(args, os.path.join(work_dir, "info.log"))

    # Prepare agent
    assert torch.cuda.is_available(), "must have cuda enabled"
    replay_buffer = utils_2.ReplayBuffer(
    ...
    kl_loss = None
    eval_array = np.zeros(args.train_steps//10000+1)    # 记录eval的reward
    cof = 1    
    for step in range(start_step, args.train_steps + 1):
        if done:
            ...
            if step % args.eval_freq == 0:
                ...
                eval_reward = evaluate(env, agent, args.algorithm, video, args.eval_episodes, L, step)
                ...
                if test_envs is not None:
                    for test_env, test_env_mode in zip(test_envs, test_envs_mode):
                        ...
                        writter.add_scalar(f'test_reward_{test_env_mode}', test_reward, step)                        
                L.dump(step)
